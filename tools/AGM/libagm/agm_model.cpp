...
std::vector<AGMModelSymbol::SPtr> AGMModel::getSymbolsByParent(AGMModelSymbol::SPtr a) const
{
	std::vector<AGMModelEdge> vEdges = getEdgesByParent(a);
	std::vector<AGMModelSymbol::SPtr> v;
	v.clear();	
	
	for(uint32_t i=0; i<vEdges.size(); i++){
		int symbolId = vEdges[i].symbolPair.second;
		int idx = getIndexByIdentifier(symbolId);
		v.push_back(symbols[idx]);
	}
...
void AGMModel::addEdgeByName(std::string aName, std::string bName, const std::string &edgeName, std::map<std::string, std::string> atr){
	int32_t a = getIdentifierByName(aName);
	int32_t b = getIdentifierByName(bName);
	
	addEdgeByIdentifiers(a, b, edgeName, atr);
}
...
std::vector<AGMModelEdge> AGMModel::getEdgesByParentIdentifier(int32_t a) const
{

	std::vector<AGMModelEdge> v;
	v.clear();
...
std::vector<AGMModelEdge> AGMModel::getEdgesByParent(AGMModelSymbol::SPtr a) const
{
	return getEdgesByParentIdentifier(a->identifier);
}
