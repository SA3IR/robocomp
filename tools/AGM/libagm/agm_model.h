...
public:
	...
	/*! \brief Returns a reference to the symbols vector that match with the symbol parent given by the param '<em>a</em>'. Use with care. If not found any, returns an empty vector.
	 */
	std::vector<AGMModelSymbol::SPtr> getSymbolsByParent(AGMModelSymbol::SPtr a) const;
...
// 	std::vector<AGMModelEdge> getEdges() const;
	...
	/*! \brief Includes a new edge in the model given the names of two symbols with an optional attribute map.  Returns True on success.
	 *
	 * \throws AGMException Nodes a and b must exist
	 *
	 */
	void addEdgeByName(std::string aName, std::string bName, const std::string &edgeName, std::map<std::string, std::string> atr=std::map<std::string, std::string>());
	...
	/*! \brief Returns a reference to the edges vector that match with the identifier of the symbol a.
	 *
	 * \throws AGMException Nodes a must exist
	 *
	 */
	std::vector<AGMModelEdge> getEdgesByParentIdentifier(int32_t a) const;
	
	/*! \brief Returns a reference to the edges vector that match with the symbol a.
	 *
	 * \throws AGMException Nodes a must exist
	 *
	 */
	std::vector<AGMModelEdge> getEdgesByParent(AGMModelSymbol::SPtr a) const;
